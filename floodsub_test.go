...
func TestNoConnection(t *testing.T) {
	...
	err = topic.Publish(ctx, []byte("TESTING"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestFloodSubPluggableProtocol(t *testing.T) {
	t.Run("multi-procol router acts like a hub", func(t *testing.T) {
		...
		topic, err := psubC.Join("foobar")
		if err != nil {
			t.Fatal(err)
		}
		topic.Publish(ctx, []byte("bar"))
...
func TestImproperlySignedMessageRejected(t *testing.T) {
	...
	tp, err := adversaryPubSub.Join(topic)
	if err != nil {
		t.Fatal(err)
	}

	// First the adversary sends the correct message.
	err = tp.Publish(ctx, correctMessage)
	if err != nil {
		t.Fatal(err)
	}
	...
	adversaryPubSub.signKey = honestPubSub.host.Peerstore().PrivKey(honestPubSub.signID.ID)
	err = tp.Publish(ctx, incorrectMessage)
	if err != nil {
		t.Fatal(err)
	}
